...
inline void split(const std::string &src, const std::string &delim,
  ...
  index = str.find_first_of(
      delim,
      start); //在str中查找(起始：start) delim的任意字符的第一次出现的位置
  ...
  while (1) {
    ...
    start = str.find_first_not_of(
        delim,
        index); //在str中查找(起始：index) 第一个不属于delim的字符出现的位置
...
public:
  ...
  void setBaseline() { baseline = undistort->getBl(); }
