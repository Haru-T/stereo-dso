...
float setting_maxShiftWeightT =
    0.04f * (640 + 480); // original is 0.04f * (640+480);
float setting_maxShiftWeightR =
    0.0f * (640 + 480); // original is 0.0f * (640+480);
float setting_maxShiftWeightRT =
...
float setting_desiredImmatureDensity =
    3000; // original is 1500 immature points per frame
float setting_desiredPointDensity =
    4000; // original is 2000 aimed total points in the active window.
...
float setting_reTrackThreshold =
    1.5; //  改到1 没有得到改善 original is 1.5 (larger = re-track more often)
...
float setting_affineOptModeA =
    1e12; // original //-1: fix. >=0: optimize (with prior, if > 0).
float setting_affineOptModeB =
    1e8; // original //-1: fix. >=0: optimize (with prior, if > 0).
