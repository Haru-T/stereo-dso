...
void EnergyFunctional::accumulateAF_MT(MatXX &H, VecX &b, bool MT) {
  if (MT) {
    red->reduce(std::bind(&AccumulatedTopHessianSSE::setZero, accSSE_top_A,
                          nFrames, std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3, std::placeholders::_4),
                0, 0, 0);
    red->reduce(std::bind(&AccumulatedTopHessianSSE::addPointsInternal<0>,
                          accSSE_top_A, &allPoints, this, std::placeholders::_1,
...
void EnergyFunctional::accumulateLF_MT(MatXX &H, VecX &b, bool MT) {
  if (MT) {
    red->reduce(std::bind(&AccumulatedTopHessianSSE::setZero, accSSE_top_L,
                          nFrames, std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3, std::placeholders::_4),
                0, 0, 0);
    red->reduce(std::bind(&AccumulatedTopHessianSSE::addPointsInternal<1>,
                          accSSE_top_L, &allPoints, this, std::placeholders::_1,
                          std::placeholders::_2, std::placeholders::_3,
                          std::placeholders::_4),
                0, allPoints.size(), 50);
...
void EnergyFunctional::accumulateSCF_MT(MatXX &H, VecX &b, bool MT) {
  if (MT) {
    red->reduce(std::bind(&AccumulatedSCHessianSSE::setZero, accSSE_bot,
                          nFrames, std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3, std::placeholders::_4),
                0, 0, 0);
    red->reduce(std::bind(&AccumulatedSCHessianSSE::addPointsInternal,
                          accSSE_bot, &allPoints, true, std::placeholders::_1,
                          std::placeholders::_2, std::placeholders::_3,
                          std::placeholders::_4),
                0, allPoints.size(), 50);
...
EFFrame *EnergyFunctional::insertFrame(FrameHessian *fh, CalibHessian *Hcalib) {
  ...
  for (EFFrame *fh2 : frames) {
    connectivityMap[(((long)eff->frameID) << 32) + ((long)fh2->frameID)] =
        Eigen::Vector2i(0, 0);
    if (fh2 != eff)
      connectivityMap[(((long)fh2->frameID) << 32) + ((long)eff->frameID)] =
          Eigen::Vector2i(0, 0);
  }
...
EFPoint *EnergyFunctional::insertPoint(PointHessian *ph) {
  ...
  // printf("the host of this PointHessian is: %d, and the num of present
...
void EnergyFunctional::dropResidual(EFResidual *r) {
  ...
  connectivityMap[(((long)r->host->frameID) << 32) + ((long)r->target->frameID)]
                 [0]--;
  nResiduals--;
  r->data->efResidual = 0;
  delete r;
