...
void CoarseTracker::makeCoarseDepthForFirstFrame(FrameHessian *fh) {
  ...
  for (int lvl = 0; lvl < 2; lvl++) {
    ...
    for (int it = 0; it < numIts; it++) {
      ...
      float *idepthl =
          idepth[lvl]; // dont need to make a temp copy of depth, since I only
...
// make depth mainly from static stereo matching and fill the holes from
// propogation idpeth map.
    ...
    for (PointHessian *ph : fh->pointHessians) {
      if (ph->lastResiduals[0].first != 0 &&
          ph->lastResiduals[0].second ==
              ResState::IN) // contains information about residuals to the last
                            ...
                            // before).
      {
        ...
        ImmaturePointStatus pt_track_right =
            pt_track->traceStereo(fh_right, K1, 1);

        float new_idepth = 0;

        if (pt_track_right == ImmaturePointStatus::IPS_GOOD) {
          ImmaturePoint *pt_track_back =
              new ImmaturePoint(pt_track->lastTraceUV(0),
                                pt_track->lastTraceUV(1), fh_right, &Hcalib);
          ...
          ImmaturePointStatus pt_track_left =
              pt_track_back->traceStereo(fh_target, K1, 0);
