...
ImmaturePointStatus ImmaturePoint::traceStereo(FrameHessian *frame, Mat33f K,
                                               bool mode_right) {
  // KRKi
  ...
  if (!std::isfinite(idepth_min_stereo) || !std::isfinite(idepth_max_stereo) ||
      (idepth_max_stereo < 0)) {
    lastTracePixelInterval = 0;
    lastTraceUV = Vec2f(-1, -1);
    return lastTraceStatus = ImmaturePointStatus::IPS_OUTLIER;
 ...
 */
ImmaturePointStatus
ImmaturePoint::traceOn(FrameHessian *frame, Mat33f hostToFrame_KRKi,
